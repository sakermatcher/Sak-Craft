...
originalSurf = pygame.image.load("engine/bak.png").convert_alpha()
...
def generate():
                ...
                if selectedBlockXZY == [x, z, y]:
                    ...
                    if matrix[y][z][x]['id'] != 'vanilla:air':
                        ...
                        if z%2 == 0: 
                            ...
                            pygame.Surface.blit(surf, textures[[y][z][x].texture], dest=(x*100, z*40 - y*30 + structureDims[2]*30))
    ...
    screen.blit(pallete[selectedBlock['id']], dest=(10,10))
    pygame.display.flip()
...
while 1:
    ...
    if True:
        ...
        if keys[pygame.KSCAN_1] == 1:
            selectedBlock = {'id':list(pallete.keys())[0],  'name':namePerID[list(pallete.keys())[0]], 'state':''}

        elif keys[pygame.KSCAN_2] == 1:
            selectedBlock = {'id':list(pallete.keys())[1],  'name':namePerID[list(pallete.keys())[1]], 'state':''}

        elif keys[pygame.KSCAN_3] == 1:
            selectedBlock = {'id':list(pallete.keys())[2],  'name':namePerID[list(pallete.keys())[2]], 'state':''}

        elif keys[pygame.KSCAN_4] == 1:
            selectedBlock = {'id':list(pallete.keys())[3],  'name':namePerID[list(pallete.keys())[3]], 'state':''}
        
        elif keys[pygame.KSCAN_5] == 1:
            selectedBlock = {'id':list(pallete.keys())[4],  'name':namePerID[list(pallete.keys())[4]], 'state':''}
